...
UK Biobank accelerometer data is analysed on the DNAnexus Research Analysis Platform, where GGIR is normally run either through the Swiss Army Knife app with a Docker image of your own, or from an RStudio session on the platform. Neither of those sets the thread variables described under [Processing time](#processing-time), so if you process many files there in parallel it is worth setting them yourself. The same applies to a plain `rocker/r-ver` container, where the BLAS is threaded and nothing limits it.
...
-   Lower value of parameter `maxNcores` which by default uses the number of available cores derived with command `parallel::detectCores()` minus 1. This might in some cases be too demanding for your operating system. Note also that `parallel::detectCores()` reports the cores of the machine, not the cores your job was given. Inside a container or a scheduled cluster job the two can differ a lot, and GGIR will then start far more processes than the job can run.
-   If GGIR is running several files at once and giving it more cores did not make it faster, or made it slower, the cause may be the thread pools inside each process. Libraries such as the BLAS that R uses for matrix computation size their thread pool from the whole machine, so several R processes running at once can each open a pool of that size, and the machine is then asked for many times more threads than it has cores. They spend their time competing rather than working. You can rule this out by setting the relevant environment variables to `1` before starting R, for example `export OMP_NUM_THREADS=1` on Linux or macOS. Which variable is honoured depends on how R was built: OpenBLAS looks at `OPENBLAS_NUM_THREADS` first, then `GOTO_NUM_THREADS`, then `OMP_NUM_THREADS`; Intel MKL looks at `MKL_NUM_THREADS`; and Accelerate on macOS looks at `VECLIB_MAXIMUM_THREADS`. Setting `OMP_NUM_THREADS` alone is enough when none of the others are set, which is the common case, but it is quietly ignored when they are. Note that this only helps when GGIR is running several processes: with `do.parallel = FALSE`, or when there is only one file to process, GGIR uses a single process and a threaded BLAS is then an advantage rather than a problem, so capping it would slow you down.
