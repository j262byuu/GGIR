HASPT = function(angle, params_sleep = NULL, ws3 = 5,
  ...
  if (HASPT.algo != "notused") {
    if (HASPT.algo == "HDCZA") { # original, default
      ...
      # hold: non-finite input (median propagates NA/NaN, runmed does not), odd k1
      # (ws3 = 60 gives k1 = 5; runmed needs an odd width, and zoo maps odd widths
      # differently), and vectors too short for one window.
      # is.finite, not !anyNA: an infinite angle passes an NA check, and
      # diff() of two equal infinities produces NaN AFTER the guard, where
      # runmed and the windowed median then disagree. Angle is bounded in
      # practice, so this is unreachable rather than a fix -- but the
      # equivalence claim below is unconditional and the guard should be too.
      use_runmed = all(is.finite(angle)) && k1 %% 2 == 0 && length(angle) - 1 >= k1 - 1
      if (use_runmed == TRUE) {
        half = k1 %/% 2
        d = abs(diff(angle))
